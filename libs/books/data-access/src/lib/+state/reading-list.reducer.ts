...
const readingListReducer = createReducer(
  ...
  on(ReadingListActions.finishFromReadingListSuccess, (state, action) =>
    readingListAdapter.updateOne({
      id: action.item.bookId, changes: { finished: true, finishedDate: action.finishedDate }
    }, state)
  ),
  on(ReadingListActions.failedFinishedFromReadingList, (state, {error}) => ({
    ...state, error
  }))
);
